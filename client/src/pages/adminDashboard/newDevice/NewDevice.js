...
const NewDevice = () => {
    ...
    const handleChangeTypUrzadzenia = (selectedOption) => {
      if(selectedOption.label === 'Smartfon' || selectedOption.label === 'Tablet') {
        ...
        if(selectedOption.label === 'Laptop') {
          setSelectedIndex(2)
        }
        else {
          setSelectedIndex(3)
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        showSuccessToast("Pomyślnie dodano nowe urządzenie!")
        window.scrollTo(0,0)
        setNazwaUrzadzenia('')
        setUslugi(uslugiSmartfon.slice())
        setSelectedIndex(0)
        ...
        if (err?.message === "Network Error") {
          ...
          showErrorToast('Wystąpił błąd podczas dodawania nowego urządzenia!')
        }
      }
    }
          ...
          <div className='mainContainer'>
            <div className='cnt'>
              ...
              <div className='newDevice'>
                  <h1 className='newDeviceTitle'>Dodaj nowe urządzenie</h1>
                  <div className='newDeviceForm'>
                    <div style={{marginRight: '20px'}}>
                      <div className='newDeviceItem'>
                        <label>Nazwa urządzenia</label>
                        <input type='text' id="nazwaUrzadzenia" placeholder='Nazwa nowego urządzenia' onChange={(e) => {setNazwaUrzadzenia(e.target.value); e.target.style.backgroundColor='white'}} value={nazwaUrzadzenia} required />
                      </div>
                      <div className='newDeviceItem'>
                    ...
                    <div className='newDeviceRight'>
                      <div></div>
                      <button type="button" onClick={handleSubmit} className='newDeviceButton'>Dodaj</button>
                    </div>
                  </div>
              </div>
            </div>
